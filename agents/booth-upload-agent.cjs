...
const queuePath = process.env.GRADSYNC_AGENT_QUEUE || path.join(__dirname, 'runtime', 'booth-upload-queue.json');
...
async function processQueue() {
  ...
  const job = queue[0];
  try {
    // 1. Fetch the active students from the booth API
    const activeStudents = await api('/api/booth/active');
    if (!activeStudents || !Array.isArray(activeStudents) || activeStudents.length === 0) {
